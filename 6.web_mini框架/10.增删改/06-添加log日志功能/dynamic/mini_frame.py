...
def index(ret):
    ...
    # 创建Connection连接
    conn = connect(host='localhost',port=3306,user='root',password='htwoo111',database='stock_db',charset='utf8')
    # 获得Cursor对象
    cs = conn.cursor()
...
def save_update_page(ret):
    ...
    stock_code = ret.group(1)
    comment = ret.group(2)
    comment_unquote = urllib.parse.unquote(comment)
    
    conn = connect(host='localhost',port=3306,user='root',password='htwoo111',database='stock_db',charset='utf8')
    cs = conn.cursor()
    sql = """update focus set note_info=%s where info_id = (select id from info where code=%s);"""
    cs.execute(sql, (comment_unquote, stock_code))
    conn.commit()
    cs.close()
    conn.close()
...
def application(env, start_response):
    ...
    logging.info("The curret file name is %s " %file_name)
    try:
        ...
        for url, func in URL_FUNC_DICT.items():
            ...
            if ret:
                ...
                logging.warning("Not found the file name")
